...
const instance = axios.create({
  baseURL: '/',
  headers:{
    ['x-auth-token']:'Beq7TkdlK4lwq4kNcrCpHMFy98sSRjYMNezNNTp4'
  }
})
// 发布订阅
class EventEmitter {
...
// 存储已发送但未响应的请求
const pendingRequest = new Set()// 发布订阅容器
const ev = new EventEmitter()
// 添加请求拦截器
instance.interceptors.request.use(async (config) => {
  const hash = location.hash // 生成请求Key
  const reqKey = generateReqKey(config, hash)
  console.log(reqKey, 'reqKey')
  if (pendingRequest.has(reqKey)) { // 如果是相同请求,在这里将请求挂起，通过发布订阅来为该请求返回结果        // 这里需注意，拿到结果后，无论成功与否，都需要return Promise.reject()来中断这次请求，否则请求会正常发送至服务器
    let res = null
    try {
